...
%% Darya's SVD algorithm

k = 14;
[U_0 S_0 V_0] = svd(zeros);
[U_1 S_1 V_1] = svd(ones);
[U_2 S_2 V_2] = svd(twos);
[U_3 S_3 V_3] = svd(threes);
[U_4 S_4 V_4] = svd(fours);
[U_5 S_5 V_5] = svd(fives);
[U_6 S_6 V_6] = svd(sixs);
[U_7 S_7 V_7] = svd(sevens);
[U_8 S_8 V_8] = svd(eights);
[U_9 S_9 V_9] = svd(nines);

zerosApprox = U_0(:,1:k)*S_0(1:k,1:k)*V_0(:,1:k)';
onesApprox = U_1(:,1:k)*S_1(1:k,1:k)*V_1(:,1:k)';
twosApprox = U_2(:,1:k)*S_2(1:k,1:k)*V_2(:,1:k)';
threesApprox = U_3(:,1:k)*S_3(1:k,1:k)*V_3(:,1:k)';
foursApprox = U_4(:,1:k)*S_4(1:k,1:k)*V_4(:,1:k)';
fivesApprox = U_5(:,1:k)*S_5(1:k,1:k)*V_5(:,1:k)';
sixsApprox = U_6(:,1:k)*S_6(1:k,1:k)*V_6(:,1:k)';
sevensApprox = U_7(:,1:k)*S_7(1:k,1:k)*V_7(:,1:k)';
eightsApprox = U_8(:,1:k)*S_8(1:k,1:k)*V_8(:,1:k)';
ninesApprox = U_9(:,1:k)*S_9(1:k,1:k)*V_9(:,1:k)';
...
for i=1:4649
    Q = Inf; %initialize large distance between matricies
    label = nan;
